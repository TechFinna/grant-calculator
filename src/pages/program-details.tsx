...
export default function ProgramDetails() {
  ...
  const program = match ? PROGRAMS.find(p => p.slug === params.slug) : null;
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <Link href="/programs"><Button className="mt-4">Back to Directory</Button></Link>
        </div>
      </Layout>
    );
  }
      ...
      <div className="bg-white min-h-screen">
        ...
        <div className="bg-slate-50 border-b py-12">
          <div className="container mx-auto px-4">
            <Link href="/programs" className="inline-flex items-center text-sm text-slate-500 hover:text-primary mb-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <Badge className={program.status === "Open" ? "bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : "bg-slate-100 text-slate-800"}>
                    {program.status}
                  </Badge>
                </div>
                <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">{program.name}</h1>
                <p className="text-xl text-slate-600 max-w-3xl">{program.description}</p>
              </div>
              <div className="flex gap-3">
                <Button size="lg" className="shadow-md">Apply Now <ExternalLink className="ml-2 h-4 w-4" /></Button>
              </div>
            </div>
          </div>
        </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                <div className="p-4 bg-slate-50 rounded-lg border">
                  ...
                  <span className="text-xl font-bold text-primary">{program.fundingMax > 0 ? formatCurrency(program.fundingMax) : "Varies"}</span>
                </div>
                <div className="p-4 bg-slate-50 rounded-lg border">
